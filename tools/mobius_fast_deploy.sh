...
if [ ! -x "${LOGS_DIR}/swarm_bot" ]; then
    t0
    cd "${SWARM}"
    # Идентичность сборки (ветка/коммит/грязь/время): её печатают
    # стартовая строка лога бота и state dump веб-интерфейса, так что
    # любой дамп точно говорит, из какого кода он получен. В не-git
    # checkout значения пусты - коммит всё равно несёт Go buildinfo.
