...
// copied synchronously because the read buffer is reused.
...
func TestGameClientTapObservesFullSession(t *testing.T) {
        ...
        collector := &tapCollector{payloads: nil}
...
func TestGameClientSendRawForwardsClientPackets(t *testing.T) {
        ...
        server.flow = func(s *fakeGameServer, conn net.Conn, cipher *crypt.GameCrypt) {
                ...
                // A full char selected packet (the parser needs the whole tail).
                var selected []byte
                ...
                // Two raw packets sent by the proxy relay must arrive intact.
                first := s.readEncrypted(conn, cipher)
                require.Equal(s.t, byte(0x01), first[0], "first raw packet opcode")
                require.EqualValues(t, 777, readLE32(first[1:]))

                second := s.readEncrypted(conn, cipher)
                require.Equal(s.t, byte(0x0D), second[0], "second raw packet opcode")
        }
        ...
        // A MoveToLocation client packet: [0x01][x][y][z][ox][oy][oz][mode].
        raw := []byte{0x01}
        ...
        require.NoError(t, client.SendRaw([]byte{0x0D, 0x00, 0x00, 0x00, 0x00}))
...
// readLE32 reads a little endian int32 value from the packet.
func readLE32(data []byte) int32 {
