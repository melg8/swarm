...
func TestBotStateEndpoint(t *testing.T) {
        ...
        // The diagnostics section rides every state response: the
        // known list summary of the fixture (one npc) and the object
        // tally must agree.
        require.Equal(t, 1, snapshot.Diagnostics.Objects.NPCs)
        require.Equal(t, 0, snapshot.Diagnostics.Objects.Items)
        require.Len(t, snapshot.Objects, snapshot.Diagnostics.Objects.NPCs+
                snapshot.Diagnostics.Objects.Players+
                snapshot.Diagnostics.Objects.Items)
}
