...
// DecodeTile parses the raw bytes of one tile file.
func DecodeTile(data []byte) (*Tile, error) {
	...
	if version := binary.LittleEndian.Uint32(data[4:]); version != tileVersion {
...
// decodePolys reads the polygon section.
func decodePolys(data []byte, offset *int, tile *Tile) error {
	...
	for i := range tile.Polys {
		...
		if poly.X0 < 0 || poly.Y0 < 0 || poly.X1 <= poly.X0 ||
			...
			return fmt.Errorf("%w: poly %d bounds %d %d %d %d", ErrBadTile,
...
// decodeLinks reads the link section and validates the chains.
...
// decodeExtLinks reads the external link section.
...
// EncodeTile serializes a tile. The derived world anchor is NOT part
// of the wire format - it recomputes from the region key.
func EncodeTile(tile *Tile) ([]byte, error) {
	...
	for i := range tile.Polys {
		base := offset + i*polyWireSize
		poly := &tile.Polys[i]
		binary.LittleEndian.PutUint32(data[base:], uint32(poly.X0))
		binary.LittleEndian.PutUint32(data[base+4:], uint32(poly.Y0))
		binary.LittleEndian.PutUint32(data[base+8:], uint32(poly.X1))
		binary.LittleEndian.PutUint32(data[base+12:], uint32(poly.Y1))
		binary.LittleEndian.PutUint16(data[base+16:], uint16(poly.H00))
		binary.LittleEndian.PutUint16(data[base+18:], uint16(poly.H10))
		binary.LittleEndian.PutUint16(data[base+20:], uint16(poly.H01))
		binary.LittleEndian.PutUint16(data[base+22:], uint16(poly.H11))
		binary.LittleEndian.PutUint32(data[base+24:], uint32(poly.FirstLink))
		data[base+28] = poly.Area
	}
	...
	for i := range tile.Links {
		...
		binary.LittleEndian.PutUint32(data[base+12:], uint32(link.T0))
		binary.LittleEndian.PutUint32(data[base+16:], uint32(link.T1))
	}
	...
	for i := range tile.ExtLinks {
		base := offset + i*extLinkWireSize
		ext := &tile.ExtLinks[i]
		binary.LittleEndian.PutUint32(data[base:], uint32(ext.Col))
		binary.LittleEndian.PutUint32(data[base+4:], uint32(ext.Row))
		binary.LittleEndian.PutUint32(data[base+8:], ext.Poly)
	}
