...
// TestMerchantSelectionFiresTheAttackAnalogPull pins the interaction
// practice of every npc talk: the merchant is not only selected - the
// attack analog pull (the second plain click on the selected npc, the
// client double click) fires once and the server walks the character
// to the merchant along the straight line, so the transaction
// distance is met with certainty. The pull is not the real attack
// request (the 0x0A packet the guard engage drives) and it fires once
// per merchant - the re-pick to a different object id re-arms it.
func TestMerchantSelectionFiresTheAttackAnalogPull(t *testing.T) {
    loop, game, bot, _ := newTripLoop()
    ariel := townMerchants[1]
    spawnMerchantNPC(bot, ariel, 61)
    moveSelfTo(bot, ariel.X, ariel.Y, ariel.Z)
    ...
    // The selection re-requests itself first (the plain attack
    // request of the unselected npc only selects it).
    now := time.Now()
    loop.merchantPick = now.Add(-2 * time.Second)
    require.False(t, loop.selectMerchant(now))
    require.Contains(t, game.forces, int32(61))
    require.Empty(t, game.pulls,
        "the pull waits for the confirmed selection")

    // The confirmed selection fires the pull exactly once.
    bot.ApplySelfTarget(61)
    require.True(t, loop.selectMerchant(now))
    require.Equal(t, []int32{61}, game.pulls,
        "the attack analog pull fires once on the confirmed selection")

    // The next tick of the same merchant does not repeat it.
    require.True(t, loop.selectMerchant(now))
    require.Equal(t, []int32{61}, game.pulls)
}

// TestMerchantDeckCaseHandsTheWalkToTheServer pins the deck wait of
// the approach: the character stands under the merchant deck (the 2D
// distance is met, the z gap keeps the interaction distance out of
// reach) and the select + attack analog pull ladder hands the close
// walk to the server - the plain click selects at any distance, the
// pull click on the selected npc takes the interact intention. The
// ground click re-walk of the old machinery stays silent here (the
// clicks cannot close a z gap) and the window give-up survives.
func TestMerchantDeckCaseHandsTheWalkToTheServer(t *testing.T) {
    loop, game, bot, _ := newTripLoop()
    logBuf := &bytes.Buffer{}
    loop.SetLogger(log.New(logBuf, "", 0))
    unoren := townMerchants[0]
    spawnMerchantNPC(bot, unoren, 60)
    // The bot stands right below the deck: 2D distance 0, z gap 300.
    moveSelfTo(bot, unoren.X, unoren.Y, unoren.Z+300)
    loop.merchantID = 60
    loop.merchantPick = time.Now().Add(-2 * time.Second)
    loop.sellPhaseAt = time.Now().Add(-time.Minute)

    // The first paced click of the ladder selects the npc.
    now := time.Now()
    require.False(t, loop.handleMerchant(now,
        loop.merchantTemplates()))
    require.Contains(t, game.clicks, int32(60),
        "the ladder starts with the plain select click")
    require.Empty(t, game.walks,
        "the deck wait does not ground click the approach")
    require.Contains(t, logBuf.String(), "the attack analog pull walks")

    // The confirmed selection hands the walk to the server.
    bot.ApplySelfTarget(60)
    loop.merchantPull = now.Add(-2 * time.Second)
    require.False(t, loop.handleMerchant(now.Add(time.Second),
        loop.merchantTemplates()))
    require.Equal(t, []int32{60}, game.pulls,
        "the pull click takes the interact intention")
    require.Empty(t, game.walks)

    // The burned window gives the merchant up (the buys of the stop
    // skip, the sale continues without it).
    loop.merchantDeckUntil = now.Add(-time.Second)
    loop.merchantPull = now.Add(-2 * time.Second)
    require.True(t, loop.handleMerchant(now.Add(2*time.Second),
        loop.merchantTemplates()))
    require.Equal(t, int32(-1), loop.merchantID)
}
