...
// TestLoopJournalKillResetsFightClock verifies the fight clock never
// outlives its fight: the kill of the target drops the anchor, so the
// recycled object id of the next respawn of the same spawn point starts
// a fresh measurement instead of inheriting the elapsed seconds (the
...
func TestLoopJournalKillResetsFightClock(t *testing.T) {
    ...
    now := time.Now()
    loop.journalKill(9, now)
    loop.resetFightClock()
    // The recycled id of the respawn fights again: the anchor is gone,
    // so the second kill measures only its own fight.
    loop.fightStartFor = 9
    loop.fightStartAt = now.Add(-6 * time.Second)
    loop.journalKill(9, now.Add(time.Second))
    journal.Close()

    records := journalRecords(t, journal.Path())
    require.Len(t, records, 2)
    require.InDelta(t, 8.0, records[0].Dur, 1.5)
    require.InDelta(t, 7.0, records[1].Dur, 1.5)
}
