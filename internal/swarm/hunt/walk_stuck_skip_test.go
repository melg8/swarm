...
// the end waypoint (not the mid one the server refused).
func TestWalkStuckSkipsCurrentWaypoint(t *testing.T) {
	...
	require.Equal(t, 1, loop.rePaths,
		"the first stuck must increment the re-path counter")
...
func TestWalkStuckRepathsAfterAllWaypointsSkipped(t *testing.T) {
	...
	// First stuck: skip from mid (wp1) to end (wp2).
	armStuck(loop, bot)
	loop.tick()
	require.Equal(t, 1, loop.rePaths)
	require.Equal(t, 2, loop.wpIndex)
	...
	// skip), so the leg re-plans from the current position.
	armStuck(loop, bot)
	nav.calls = 0
	loop.tick()
...
// reason. The skip and re-path share the same budget, so the trip
// aborts after maxRePaths exhausted attempts.
