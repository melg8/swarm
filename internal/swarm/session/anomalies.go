...
func (a *anomalyBot) applySample(r record) {
        if a.lastSample != 0 && r.T-a.lastSample >= gapReportSec {
                a.gaps = append(a.gaps, gapMark{t: a.lastSample, sec: float64(r.T - a.lastSample)})
        }
        a.lastSample = r.T
}
...
func (a *anomalyBot) slowMobs() []*mobAnomaly {
        ...
        sort.Slice(mobs, func(i, j int) bool { return mobs[i].durMax > mobs[j].durMax })
...
func (r *anomalyRender) patternFindings(a *anomalyBot) []finding {
        var findings []finding
        for _, stat := range a.topPatterns() {
                severity := sevMedium
                if stat.count >= 100 {
                        severity = sevHigh
                }
...
func (r *anomalyRender) deathFindings(a *anomalyBot) []finding {
        var findings []finding
        for _, burst := range a.deathBursts() {
