...
const (
    ...
    // kindLogout is one intentional emergency logout of the hunt
    // safety layer (the mob pile up, the critical health escape):
    // the honest reason behind the socket close the supervisor
    // would otherwise book as a lost connection.
    kindLogout = "logout"
)
...
type record struct {
    ...
    // Ti is the login cooldown seconds of an intentional logout.
