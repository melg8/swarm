...
func TestSkillPlanJSONShape(t *testing.T) {
        ...
        require.Equal(t, float64(142), skill["skillId"])
        require.Equal(t, float64(1), skill["level"])
        ...
        require.True(t, ok)
        require.Equal(t, float64(200), plan["sp"])
        ...
        require.True(t, ok)
        require.Equal(t, "Power Strike", first["name"])
        require.Equal(t, float64(60), first["spCost"])
        require.Equal(t, float64(5), first["reqLevel"])
        require.Equal(t, false, first["passive"])
        require.Equal(t, true, first["affordable"])
        require.Equal(t, float64(0), first["category"])
}
