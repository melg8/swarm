...
func appendHuntDiagnosticsJSON(dst []byte, h HuntDiagnostics) []byte {
    ...
    dst = append(dst, `,"delevelActive":`...)
    dst = strconv.AppendBool(dst, h.DelevelActive)
    dst = append(dst, `,"delevelTarget":`...)
    dst = strconv.AppendInt(dst, int64(h.DelevelTarget), 10)
    dst = append(dst, `,"delevelFromLevel":`...)
    dst = strconv.AppendInt(dst, int64(h.DelevelFromLevel), 10)
    dst = append(dst, `,"delevelZoneMedian":`...)
    dst = strconv.AppendInt(dst, int64(h.DelevelZoneMedian), 10)
